...
let token_formtrace = "jh71r7qq5racqjc6qyrqr0n7ph7mgkw8";
...
if (scriptElement) {
  ...
  token_formtrace = "jh71r7qq5racqjc6qyrqr0n7ph7mgkw8";
  ...
  keepVideo_formtrace = urlParams.get("keepVideo")
    ? urlParams.get("keepVideo")
    : false;
  ...
  saveOnSubmit_formtrace = urlParams.get("saveOnSubmit")
    ? urlParams.get("saveOnSubmit")
    : true;
} else {
...
if (automaticRecord_formtrace) {
  console.log("formTrace v.1.0.23 initialized");
  ...
  if (debug_formtrace && guide_formtrace) {
    ...
    alertMessage += redirectValue_formtrace
      ? ` | redirect: ${redirectValue_formtrace}`
      : ` | no redirect`;
...
async function _ft_runAsyncPipeline(evt) {
  ...
  if (esp_formtrace === false) {
    if (
      tfaTwilio_formtrace &&
      tfaTwilio_formtrace === true &&
      blackList_formtrace === false
    ) {
      await tfaValidation(
        tfaTwilio_formtrace,
        phoneInputId_formtrace,
        sendTfaCodeApi,
        validateTfCodeApi,
        saveOnSubmit_formtrace,
        evt
      );
    } else if (blackList_formtrace && blackList_formtrace === true) {
      await blackListPhone(
        tfaTwilio_formtrace,
        blackList_formtrace,
        phoneInputId_formtrace,
        validateBlackListApi,
        saveOnSubmit_formtrace,
        evt
      );
    } else {
...
async function formTraceSaveRecordWithOnsubmitEvent(data) {
  ...
  if (termsText) {
    ...
    console.log("📋 Términos detectados:", data["terms"]);
  }
  ...
  const searchForm =
    document.getElementById("formproofScript")?.closest("form") ||
  ...
  try {
    ...
    const status =
      !recordingIdFromBrowser && guide_formtrace
        ? "partial"
        : redirectId_formtrace && !guide_formtrace
        ? "partial-followMe"
        : "completed";
    ...
    const dataSubmit = {
      ...
      clientIp,
      userAgent,
      token: token_formtrace || "jh71r7qq5racqjc6qyrqr0n7ph7mgkw8", // ← Para la validación de Convex
...
async function formTraceSaveRecord(data = {}) {
  ...
  let formTraceIdValue =
    recordingIdFromBrowser ||
    ((guide_formtrace || redirectId_formtrace) && generateUUID());
  const searchForm =
    document.getElementById("formproofScript")?.closest("form") ||
    document.querySelector("form");
  ...
  try {
    ...
    const eventsToSubmit = events_formtrace;
    const status =
      !recordingIdFromBrowser && guide_formtrace ? "partial" : "completed";

    console.log("📊 Status de la grabación:", status);
    const dataSubmit = {
      form: data,
      events: JSON.stringify(eventsToSubmit),
      clientIp,
      userAgent,
      token: token_formtrace || "jh71r7qq5racqjc6qyrqr0n7ph7mgkw8",
      status: status,
    };
